"""The screening topology, declared once and executed by either engine.

The nodes, the edges, the routers and the required-stage rule live here and
nowhere else. ``LangGraphWorkflow`` compiles this into a real
...
test meaningful. Both engines call *the same* router functions and *the same*
``missing_required`` check, so when the test asserts that a subject produces an
identical path, memo and recommendation under both, it is asserting that the
two executors agree - not that two hand-written copies of the graph happen to
have been edited in step.
